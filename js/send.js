'use strict';
{
  const warpControl = document.querySelectorAll(`.pay-modal__control-wrap`);
  const btnSend = document.querySelector(`.success--js2`);
  const control = document.querySelectorAll(`.pay-modal__control`);
  ...
  btnSend.addEventListener(`click`, (evt) => {
    ...
    if (control[0].value !== `` && re.test(control[0].value)) {
      warpControl[0].classList.remove(`pay-modal__control-wrap--active`);
      dagger1 = true;
    } else {
      warpControl[0].classList.add(`pay-modal__control-wrap--active`);
      dagger1 = false;
    }
    if (control[1].value !== ``) {
      dagger2 = false;
      if (re2.test(control[1].value)) {
        warpControl[1].classList.remove(`pay-modal__control-wrap--active`);
        dagger2 = true;
      } else {
        warpControl[1].classList.add(`pay-modal__control-wrap--active`);
        dagger2 = false;
      }
    } else {
      warpControl[1].classList.remove(`pay-modal__control-wrap--active`);
